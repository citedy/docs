...
Not in public MCP v1 (still available in Agent API): `PUT /api/agent/settings`, `GET /api/agent/schedule/suggest`, `POST /api/agent/shorts/script`, `POST /api/agent/shorts/avatar`, `POST /api/agent/shorts/merge`.
...
API availability: yes.
MCP v1 public exposure: no (internal pipeline step).
...
- `web_article`: 1 cr
- `pdf_document`: 2 cr
- `youtube_video`: 5-55 cr (by duration)
- `audio_file`: 3-30 cr (by duration)
