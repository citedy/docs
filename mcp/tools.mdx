...
| Tool | Description | Endpoint |
|---|---|---|
| `article.generate` | Start article generation via autopilot. This tool usually returns a queued article id first; always poll article.get with that id until status is completed or failed. | `POST /api/agent/autopilot` |
